...
def run_single_go(cfg: dotdict, data_generator: Optional[RandomDatasetGenerator], mini_run: int = 1, num_mini_runs: int = 1):
    ...
    t_type = torch.float32
    auto_encoder = EncoderDecoder(cfg.mlp_width, cfg.n_components_dictionary, t_type, l1_coef=cfg.l1_alpha).to(device)
...
def run_with_real_data(cfg, auto_encoder: EncoderDecoder, completed_batches: int = 0, mini_run: int = 1, n_mini_runs: int = 1):
    optimizer = optim.Adam(auto_encoder.parameters(), lr=cfg.learning_rate)
    running_recon_loss = 0.0
    running_l1_loss = 0.0
    import collections
...
def run_different_target(cfg, auto_encoder: EncoderDecoder, completed_batches: int = 0, mini_run: int = 1, n_mini_runs: int = 1):
    optimizer = optim.Adam(auto_encoder.parameters(), lr=cfg.learning_rate)
    running_recon_loss = 0.0
    running_l1_loss = 0.0
    import collections
    ...
    time_horizon = 1000
    # torch.autograd.set_detect_anomaly(True)
    if "target_dataset_folder" in cfg:
        target_dataset_folder = cfg.target_dataset_folder
    else:
        target_dataset_folder = cfg.dataset_folder
    n_chunks_in_folder = min(len(os.listdir(cfg.dataset_folder)), len(os.listdir(target_dataset_folder)))
    # wb_tag = f"l1={cfg.l1_alpha:.2E}_ds={cfg.n_components_dictionary}"
    wb_tag = ""
    old_dict = auto_encoder.decoder.weight.detach().cpu().data.t().clone()
    n_batches = 0
    ...
    for epoch in range(cfg.epochs):
        chunk_order = np.random.permutation(n_chunks_in_folder)
        for chunk_ndx, chunk_id in enumerate(chunk_order):
            input_chunk_loc = os.path.join(cfg.dataset_folder, f"{chunk_id}.pkl")
            target_chunk_loc = os.path.join(target_dataset_folder, f"{chunk_id}.pkl")
            dataset = DataLoader(pickle.load(open(input_chunk_loc, "rb")), batch_size=cfg.batch_size, shuffle=False)
            dataset_target = DataLoader(pickle.load(open(target_chunk_loc, "rb")), batch_size=cfg.batch_size, shuffle=False)
            for batch_idx, (batch, batch_out) in enumerate(zip(dataset, dataset_target)):
                n_batches += 1
                batch = batch[0].to(cfg.device).to(torch.float32)
                batch_out = batch_out[0].to(cfg.device).to(torch.float32)
                optimizer.zero_grad()
                # Run through auto_encoder

                # x_hat, dict_levels = auto_encoder(batch + cfg.l1_alpha * torch.randn_like(batch)) #TODO change noise thing
                x_hat, dict_levels = auto_encoder(batch)
                l_reconstruction = torch.nn.MSELoss()(batch_out, x_hat)
                l_l1 = cfg.l1_alpha * torch.norm(dict_levels, 1, dim=1).mean()
                # l_l1 = cfg.l1_alpha * torch.norm(dict_levels, 0.5, dim=1).mean() # l0.5
                # l_l1 = cfg.l1_alpha * (torch.norm(dict_levels, 1, dim=1).mean() / torch.norm(dict_levels, 2, dim=1).mean()) # l2/l1
                # exp_x = torch.exp(dict_levels)
...
def setup_data(cfg, tokenizer, model, use_baukit=False, start_line=0):
    ...
    tensor_name = "transformer.h.0.mlp"
...
def run_real_data_model(cfg: dotdict):
    ...
    if cfg.model_name in ["gpt2", "EleutherAI/pythia-70m-deduped", "EleutherAI/pythia-1.4b-deduped", "EleutherAI/pythia-12b-deduped", "usvsnsp/pythia-2.8b-sft"]:
    ...
    # 2D array of learned dictionaries, indexed by l1_alpha and learned_dict_ratio, start with Nones
    auto_encoders = [[EncoderDecoder(cfg.mlp_width, n_feats, l1_coef=l1_ndx).to(cfg.device) for n_feats in dict_sizes] for l1_ndx in l1_range]
